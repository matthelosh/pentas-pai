...
const menus = ref([
    ...
    },
    {
        ...
        children: [
            ...
            {
                label: "Form Nilai",
                url: "dashboard.lomba.nilai.form",
                roles: ["admin", "panitia"],
                icon: "edit",
            },
            {
...
<template>
    <ul class="mt-2 bg-white bg-opacity-20 overflow-y-auto h-[60%]">
        <li
            ...
            :key="i"
            v-show="menu.roles.includes(role)"
        >
            <Link
                :href="menu.url !== '#' ? route(menu.url) : '#'"
                class="flex items-center py-2 px-3 md:hover:bg-opacity-70 md:hover:bg-white md:hover:shadow md:hover:pl-5 transition-all duration-500 font-extralight"
                :class="route().current() == menu.url ? 'active' : ''"
                v-if="!menu.children"
            >
            ...
            <a
                v-else
                href="#"
                class="parent flex items-center py-2 px-3 md:hover:bg-opacity-70 md:hover:bg-white md:hover:shadow md:hover:pl-5 transition-all duration-500 font-extralight"
                @click.prevent="expandChild"
                :class="
                    doesMyChildActive([
                        'dashboard.peserta',
                        'dashboard.guru',
            ...
            <ul
                class="child-menu duration-500 transition-all ease-in-out shadow-inner bg-white bg-opacity-50"
                :class="doesMyChildActive(menu.children) ? 'inline' : 'hidden'"
                v-if="menu.children"
            >
                <li
                ...
                >
                    <Link
                        :href="child.url !== '#' ? route(child.url) : '#'"
                        class="flex items-center gap-1 hover:bg-white md:hover:bg-opacity-70 px-8 py-2 font-extralight"
                        :class="
                            amIActive(child.url)
