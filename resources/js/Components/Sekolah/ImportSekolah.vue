...
const impor = async () => {
    loading.value = true;
    await axios
        .post(route("sekolah.impor"), { sekolahs: JSON.stringify(props.datas) })
        .then((res) => {
            loading.value = false;
...
<template>
    ...
    <div
        class="fixed top-0 right-0 bottom-0 left-0 bg-gray-800 flex items-center justify-center z-40 bg-opacity-70"
        v-if="show"
    >
        <div class="card w-full md:w-3/4 bg-white rounded overflow-hidden">
            <div
                class="card-toolbar h-12 w-full shadow flex items-center justify-between p-2 bg-sky-800 text-white"
            >
                Impor Data Sekolah
                <div class="toolbar-items flex items-center">
                    <button
                        class="bg-sky-600 hover:bg-sky-400 active:bg-orange-600 text-white px-2 py-1 rounded shadow"
                        @click="impor"
                    >
                        Kirim
                    </button>
            ...
            <div class="card-content">
                <table class="table w-full border border-collapse">
                    ...
                    <tbody>
                        <tr
                            ...
                            v-for="(data, d) in pagedatas.current"
                            :key="d"
                        >
                ...
                <div
                    class="w-full bg-gray-200 flex items-center justify-between pl-3"
                >
                    Jml Halaman: {{ pagedatas.pageCount }} | Jml Data:
                    {{ pagedatas.dataLength }}
                    <div class="flex items-center h-full flex-wrap">
                        ...
                        <button
                            v-for="b in pagedatas.pageCount"
                            :key="b"
                            class="flex justify-center w-8 border border-gray-500"
                            :class="
