...
<template>
    <div class="content w-full bg-white">
        ...
        <div
            class="toolbar h-12 px-2 flex items-center justify-between print:hidden shadow-lg"
        >
            <div class="toolbar-title">Presensi Panitia</div>
            <div class="toolbar-items flex items-center gap-2">
                <SvgIcon
                    type="mdi"
                    :path="mdiClose"
                    class="w-10 text-red-600 hover:text-red-800 active:text-orange-600 rounded-full cursor-pointer"
                    @click="$emit('close')"
                />
            </div>
        </div>
            ...
            <table class="w-full border border-collapse border-black">
                ...
                <thead>
                    <tr>
                        ...
                        <th class="py-2 px-3 border border-black">
                            Tanda Tangan
                        </th>
                    </tr>
                </thead>
                <tbody>
                    <tr v-for="(data, d) in datas" :key="d">
                        <td class="border border-black text-center p-2 w-[5%]">
                            {{ d + 1 }}
                        </td>
