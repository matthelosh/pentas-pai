...
const items = ref([]);
const list = async () => {
    await axios.post(route("rekap")).then((res) => {
...
<template>
    <div class="w-full bg-white rounded-xl mt-3 md:mt-0">
        ...
        <div
            class="toolbar p-3 flex items-center justify-between sticky top-0 bg-gray-100 shadow rounded-t-xl"
        >
            Rekapitulasi Peserta
            <div class="toolbar-items flex items-center">
                <button
                    class="rounded-full bg-red-400 p-1 hover:rotate-45 transition-transform duration-200"
                    @click="$emit('close')"
                >
                    <SvgIcon
                        type="mdi"
        ...
        <div class="content bg-white p-3 w-full overflow-auto">
            <table class="border border-separate border-spacing-1 w-full">
                ...
                <tbody>
                    <tr
                        v-for="(data, d) in items"
                        class="break-inside-avoid-page"
                    >
                        <td class="border border-black w-[5%] text-center">
                            {{ d + 1 }}
                        </td>
                        <td class="border border-black w-[25%] p-4">
                            {{ data.nama }}
                        </td>
                        <td class="border border-black p-1 text-cent">
                            <table class="w-full">
                                <thead>
                                    <tr>
                                        <th class="border p-2 border-black">
                                            No
                                        </th>
                                        <th class="border p-2 border-black">
                                            NISN
                                ...
                                <tbody>
                                    <tr v-for="(siswa, s) in data.pesertas">
                                        <td
                            ...
                            <!-- <span
                                class="w-full flex my-2 gap-2 border"
                                v-for="(peserta, pe) in perBidangs(
                                    data.pesertas
                                )"
                                :key="pe"
                            >
                                <div
                                    class="bg-gray-50 p-2 w-[30%] flex items-center"
                                >
                                    {{ pe }}
                                </div>
                                <div
                                    class="bg-teal-50 w-[70%] flex items-center px-2 flex-grow-1"
                                >
                                    <ul class="w-full" v-if="peserta">
                                        <li
                                            v-for="(item, it) in peserta"
                                            :key="it"
                                        ...
                                        >
                                            ...
                                            <img
                                                ...
                                                alt=""
                                                class="w-12 aspect-square rounded-full object-cover object-top hidden md:block"
                                            />
                                        </li>
                                    </ul>
                                </div>
