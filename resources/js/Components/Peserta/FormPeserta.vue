...
const onFotoPicked = (e) => {
    ...
    if (!file) {
        ...
        new Compressor(file, {
            ...
            success(result) {
                urlFoto.value = URL.createObjectURL(result);
                fileFoto.value = result;
            },
            error(err) {
                console.log(err);
            },
...
const listBidangs = () => {
    axios.post(route("bidang.index")).then((res) => {
...
<template>
    <div
        class="fixed top-0 right-0 bottom-0 left-0 bg-[#33333389] flex items-center justify-center z-50"
        @click.self="close"
        v-if="show"
    >
        <div
            class="grid grid-cols-1 md:grid-cols-2 md:p-10 md:gap-5 relative bg-gray-50 w-[90vw] h-90vh md:w-2/4"
        >
            <div
                class="w-full bg-gray-200 p-3 mb-0 md:mb-3 rounded flex flex-col justify-center items-center relative md:static"
            >
                <SvgIcon
                    type="mdi"
                    :path="mdiClose"
                    class="absolute w-10 text-gray-400 hover:text-gray-600 active:text-white top-4 right-2"
                    @click="close"
                />
                <img
                    :src="urlFoto"
                ...
                <h1 class="text-center text-gray-500 text-lg">
                    ...
                    <small class="text-black"
                        >[Klik gambar untuk mengganti]</small
                    >
                </h1>
                <input
                    type="file"
            ...
            <form
                ...
                class="p-2 h-auto"
            >
                <h3 class="text-lg text-gray-700 mt-0 mb-4 font-extrabold">
                    Data Peserta
                </h3>
                <div class="flex justify-between mb-3">
                    ...
                    <select
                        name="sekolah"
                        class="rounded-lg w-3/4 p-1 placeholder:text-blue-500 focus:border-blue-500 bg-gray-100"
                        placeholder="Pilih"
                        v-model="peserta.sekolah_id"
                    >
                        <option value="0">Pilih Sekolah</option>
                        <option
                            v-for="(sekolah, s) in page.props.sekolahs"
                            :key="s"
                            :value="sekolah.npsn"
                        >
                            {{ sekolah.nama }}
                ...
                <div class="flex justify-between mb-3">
                    ...
                    <select
                        ...
                        class="rounded-lg w-3/4 p-1 placeholder:text-blue-500 focus:border-blue-500 bg-gray-100"
                        placeholder="Pilih"
                        v-model="peserta.bidangIds"
                        multiple
                    >
                        <!-- <option value="0" selected>Pilih</option> -->
                        <option
                            v-for="(bidang, l) in bidangs"
                            :key="l"
                            :value="bidang.id"
                        >
                ...
                <div class="flex justify-center mb-3">
                    <button
                        type="submit"
                        class="bg-blue-600 text-white rounded px-3 py-1 hover:bg-blue-800 flex items-center disabled:bg-blue-300 disabled:text-gray-300"
                        :disabled="loading"
                    >
                        <ArrowPathIcon
                            class="animate-spin w-5 mr-1"
                            v-if="loading"
                        />
